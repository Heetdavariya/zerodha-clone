...
// Fetches user's live portfolio data and passes it as context
...
router.post("/chat", verifyToken, async (req, res) => {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return res.status(503).json({ message: "AI service not configured. Please add GEMINI_API_KEY to backend .env" });
    }

    // ── Fetch user's full portfolio in parallel ────────────
    ...
    const holdingsSummary = holdings.length
      ? holdings
          .map(
            (h) =>
              `  - ${h.name}: ${h.qty} shares, avg ₹${h.avg.toFixed(2)}, current ₹${h.price.toFixed(
                2
    ...
    const positionsSummary = positions.length
      ? positions
          .map(
            (p) =>
              `  - ${p.name}: ${p.qty} shares (${p.product}), avg ₹${p.avg.toFixed(
                2
              )}, current ₹${p.price.toFixed(2)}, P&L ₹${((p.price - p.avg) * p.qty).toFixed(2)}`
          )
          .join("\n")
      : "  None";
    ...
    const ordersSummary = recentOrders.length
      ? recentOrders
          .map(
            (o) =>
              `  - ${o.mode} ${o.qty} ${o.name} @ ₹${o.price} (${o.product || "CNC"}, ${o.status})`
          )
          .join("\n")
      : "  None";

    const watchlistSummary = watchlist.length
      ? watchlist.map((w) => `  - ${w.name} @ ₹${w.price?.toFixed(2) || "N/A"}`).join("\n")
      : "  None";
