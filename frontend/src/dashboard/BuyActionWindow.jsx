...
export default function BuyActionWindow({ stockName, onClose }) {
  ...
  useEffect(() => {
    api.get("/dashboard/funds").then(({ data }) => setFunds(data.funds));
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Trigger refresh FIRST so pages update, then close after short delay
      triggerRefresh();
  ...
  return (
    <div className="action-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="action-window buy" style={{ borderRadius: "16px 16px 0 0", padding: 28, width: 460, background: "#fff", boxShadow: "0 -4px 30px rgba(0,0,0,0.15)" }}>
